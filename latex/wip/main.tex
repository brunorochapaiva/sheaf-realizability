...
\Cref{prop:boxed-partial-extension} as saying that the extension
%
\(
...
\begin{defn}\label{defn:tripos-ordering}
  ...
  we define the \definiendum{evidence predicate for \(\phi\leq\psi\)} as
  follows:
  %
  \begin{align*}
    \IEvidence{\phi}{\psi}
...
\begin{proof}
  ...
  we use \Cref{prop:stable-predicate-induction} and
  the preceding \Cref{prop:extension-of-evidence-predicate}.
  %
...
\begin{defn}
  We define the \definiendum{extension of a realizability predicate
    \(\phi : \RealPred{X}\)}
\end{defn}
